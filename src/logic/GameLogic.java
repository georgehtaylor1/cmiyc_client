...
public class GameLogic {
    ...
    public void update() {
        ...
        double angle = Maths.angle(
                client.player.position.x * vals.scaleFactor + vals.xOffset,
                client.player.position.y * vals.scaleFactor + vals.yOffset,
                mouseX, mouseY);
        ...
        if (keys.containsKey(KeyCode.W) && keys.get(KeyCode.W)
            ...
            if (Math.pow(client.player.position.x * vals.scaleFactor
                    ...
                    + Math.pow(client.player.position.y * vals.scaleFactor
                            + vals.yOffset - mouseY, 2) <= Math
                                    .pow(GameSettings.Player.radius
                                            * vals.scaleFactor, 2)) {
        ...
        if (faction == Faction.THIEF) { // Thief functions
            if (keys.containsKey(KeyCode.SPACE) && keys.get(KeyCode.SPACE)
                ...
                if (GameSettings.Arena.exit.at(client.player.position, 20)) {
                    ...
                    client.send(
                            new Transferable(Action.UPDATE_CLIENT_STATE, map));
                } else {
            ...
            if (keys.containsKey(KeyCode.SPACE) && keys.get(KeyCode.SPACE)) {
                ...
                for (Map.Entry<String, Player> p : client.gameData.players
                        .entrySet()) {
                    ...
                    if (Math.pow(px - tx, 2) + Math.pow(py - ty, 2) <= Math
                            .pow(GameSettings.Security.catchRadius, 2)) {
            ...
            if (chargingArea.contains(client.player.position.x,
